...
/**
 * Find entry points based on the environment for Vite
 * @param {Object} env - Environment settings
 * @returns {Object} Map of entry points
 */
export function findViteEntries(env) {
  let entries = {};
  ...
  // Dev mode specific handling for Vite (important for correct module loading)
  if (env.isDevelopment) {
    // Format entries to ensure they load correctly in dev mode
    Object.keys(entries).forEach(entryKey => {
      if (typeof entries[entryKey] === 'string') {
        // Remove ./ prefix which can cause issues with Vite imports
        entries[entryKey] = entries[entryKey].replace(/^\.\//, '');
      }
    });
    
    console.log('Dev mode entries:', Object.keys(entries));
  }
...
function createIsolationWrapper(componentName) {
  // Reuse the same function from rollup/entries.js
