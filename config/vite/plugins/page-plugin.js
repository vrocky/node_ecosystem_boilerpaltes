...
/**
 * Create page plugin for MPA mode in Vite
 * @param {Object} entries - Entry points
 * @param {Object} env - Environment settings
 * @returns {Object} Page generator plugin
 */
export function createVitePagePlugin(entries, env) {
  return {
    name: 'vite-plugin-generate-html-pages',
    
    resolveId(source, importer) {
      // For direct page entry requests, let default resolver handle it
      return null;
    },
    
    configureServer(server) {
      // We'll use the dev middleware plugin for development mode,
      // so this plugin's server middleware won't be used
      return () => {};
    },
    
    // Hook into the build process to generate all HTML files
    closeBundle() {
      // Only run in build mode, not in development mode
      if (env.isDevelopment) return;
      
      console.log('Generating HTML pages for MPA mode...');
      ...
      Object.keys(entries).forEach(pageName => {
        ...
        // Check if page-specific CSS exists, otherwise use the page name directly
        const cssPath = getRelativePath(htmlFileDir, `/assets/css/${pageName}.css`);
        ...
        let pageHtml = templateHtml
          ...
          // Update script src to point to this page's bundle with correct relative path
          .replace(/<script.*?src=".*?"><\/script>/, `<script type="module" src="${jsPath}"></script>`)
          // Update favicon path
          .replace(/href="favicon.png"/, `href="${faviconPath}"`);
        
        // Always add the CSS reference - use the page's name for CSS
